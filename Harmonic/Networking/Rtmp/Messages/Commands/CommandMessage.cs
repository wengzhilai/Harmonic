    ...
    public abstract class CommandMessage : Message
    {
        ...
        public void DeserializeAmf0(SerializationContext context)
        {
            ...
            while (buffer.Length > 0)
            {
                ...
                if ("PublishingName".Equals(optionArguments[i].Name) && optArg.ToString().IndexOf("?pwd=")>0)
                {
                    var pwdArr = GetType().GetProperties().SingleOrDefault(x => x.Name == "pwd");
                    pwdArr.SetValue(this, optArg.ToString().Split("?pwd=")[1]);
                    optArg = optArg.ToString().Split("?pwd=")[0];
                }
