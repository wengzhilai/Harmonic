    ...
    public class RtmpSession : IDisposable
    {
        ...
        internal void CommandHandler(RtmpController controller, CommandMessage command)
        {
            ...
            try
            {
                ...
                if(command is PublishCommandMessage)
                {
                    PublishCommandMessage tmp = (PublishCommandMessage)command;

                    IOPipeline.Options.BackIOPipeLine?.Invoke(tmp.PublishingName, IOPipeline);

                    if (IOPipeline.Options.CheckPwd != null && !IOPipeline.Options.CheckPwd(tmp.PublishingName, tmp.pwd))
                    {
